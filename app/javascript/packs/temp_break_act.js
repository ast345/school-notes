...
export function tempBreakActivity(schoolClassId) {
            ...
            function createBreakActEndHandler(event) {
                ...
                if(!creatingElement.is(clickedElement) && creatingElement.has(clickedElement).length === 0){
                    ...
                    if (!newBreakAct) {
                        ...
                        axios.post(`/school_classes/${schoolClassId}/template_break_activities`, {
                            temp_break_act: {day_of_week: dayOfWeek, activity_name: newBreakAct}
                        })
                        .then((res) =>{
                            if(res.status === 200){
            ...
            function editBreakActEndHandler(event) {
                ...
                if(!creatingElement.is(clickedElement) && creatingElement.has(clickedElement).length === 0){
                    ...
                    if (!editBreakAct) {
                        ...
                        axios.delete(`/school_classes/${schoolClassId}/template_break_activities/${breakActId}`)
                        .then((res) =>{
                            if(res.status === 204){
                                statusDisplay.innerHTML = "保存済み"
                            };
                        ...
                        adjustFontSize(breakActDisplay);
                        axios.patch(`/school_classes/${schoolClassId}/template_break_activities/${breakActId}`, {
                            temp_break_act: {activity_name: editBreakAct}
                        })
                        .then((res) =>{
                            if(res.status === 200){
                                statusDisplay.innerHTML = "保存済み"
